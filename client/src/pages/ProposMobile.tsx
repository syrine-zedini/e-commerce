...
<main className="space-y-6 px-4 md:px-6">
      ...
      <section className="space-y-2">
        <h2 className="text-black font-bold text-lg leading-6">
          YJ Para est née de la volonté de rendre les soins parapharmaceutiques plus accessibles, plus proches des gens et parfaitement adaptés aux besoins de chacun.
        </h2>
        ...
        <ScrollArea className="w-full">
          <div className="flex gap-3 overflow-x-auto pb-4">
            {brandLogos.map((brand, idx) => (
              <Card key={idx} className="flex-shrink-0 w-60 h-40 rounded-lg bg-card-background flex items-center justify-center border border-slate-100/50 hover:border-[#1D8EE6]/25 hover:shadow-md active:scale-95 transition-all duration-300 group cursor-pointer">
      ...
      <section className="space-y-4">
        {features.map((feat, idx) => (
          <Link key={idx} to={feat.link} className="block cursor-pointer">
            <Card className="flex items-center gap-4 p-4 rounded-lg bg-card-background border border-transparent hover:border-[#1D8EE6]/20 hover:shadow-md transition-all duration-300 group">
              <div className="w-16 h-16 bg-text-light rounded-full flex items-center justify-center flex-shrink-0 group-hover:bg-[#1D8EE6]/10 transition-colors duration-300">
                <img src={feat.icon} alt={feat.title} className="w-10 h-10 transition-transform duration-300 group-hover:scale-110" />
              </div>
              <div>
                <h3 className="font-semibold text-text-dark text-sm group-hover:text-[#1D8EE6] transition-colors">{feat.title}</h3>
